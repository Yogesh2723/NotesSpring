...
public class NoteLabRepository {
    ...
    public Iterable<NoteLab> findAll() {
        return jdbcOperations.query("Select * from note_lab", new NoteLabMapper());
    }

    public NoteLab findOne(long id) {
        return jdbcOperations.queryForObject("Select * from note_lab where id = ?", new NoteLabRepository.NoteLabMapper(), id);
    }

    public List<NoteLab> findAllFromUser(String username) {
        return jdbcOperations.query("select * from note_lab where owner = ?", new NoteLabMapper(), username);
    }
    ...
    private final class NoteLabMapper implements RowMapper<NoteLab> {
        ...
        public NoteLab mapRow(ResultSet resultSet, int i) throws SQLException {
            return new NoteLabBuilder()
                    ...
                    .setTime(resultSet.getTimestamp("date_creation").toLocalDateTime())
